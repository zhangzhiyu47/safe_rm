...
pub const ARCHIVE_CAPACITY: usize = 100;
...
fn restore_item_internal(item: &RestoreItem) -> Result<()> {
    ...
    // Determine the source path of the file to restore,
    // also track the index_dir for cleanup (non-archived items only).
    let (src_path, maybe_index_dir): (PathBuf, Option<PathBuf>) = 
        if item.is_archived {
            let temp_dir = tempfile::tempdir()?;
            ...
            let index_dir = match &item.index_dir {
                Some(dir) => dir.clone(),
                None => 
                    find_index_dir(&rubbish_bin, &item.timestamp, &item.filename)?,
            };
            let src = index_dir.join(RUBBISH_SUBDIR).join(&item.filename);
            (src, Some(index_dir))
        };

    let dst_path = item.original_path.join(&item.filename);

    if let Some(parent) = dst_path.parent() {
        create_dir_all(parent)?;
    }

    // Move to destination, automatically renaming if a file already exists there
    let _final_dst = move_to_unique_path(&src_path, &dst_path)?;

    // If restored from an archive, completely remove its manifest entry
    if item.is_archived {
        remove_archive_item_from_manifest(item)?;
    }

    // Clean up empty directories after restoring a non-archived item
...
fn extract_from_archive(item: &RestoreItem, temp_dir: &Path) -> Result<PathBuf> {
    ...
    for entry in archive.entries()? {
        ...
        if path_str.contains(&format!("{}/", item.timestamp))
            && path_str.contains(&format!("/{}/", RUBBISH_SUBDIR))
        {
